...
class MVSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            # Read image
            img_filename = os.path.join(self.datapath, 'Rectified_raw/{}/rect_{:0>3}_3_r5000.jpg'.format(scan, vid))  # 1600x1200
            img = self.read_img(img_filename)
            
            # Read camera parameters
            proj_mat_filename = os.path.join(self.datapath, 'Cameras/{:0>8}_cam.txt'.format(vid)) 
            intrinsics, extrinsics, depth_min, depth_interval = self.read_cam_file(proj_mat_filename, interval_scale=self.interval_scale[scene_name]) # divided by 4: 1600x1200 --> (400x300)
